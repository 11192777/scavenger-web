    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-city="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item :label="'所在城市'">
          <el-select id="citySelect" v-model="staffEntity.cityId" :default-first-option="true" style="width: 280px" class="filter-item">
...
export default {
  ...
  methods: {
    getProvinceList(){
      SiteApi.getSiteList("PROVINCE").then( res => {
        this.provinceList = res.data
      })
    },
    getCityList(){
      SiteApi.getSiteByParentId(this.staffEntity.provinceId).then( res => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          CourierApi.createCourier(this.staffEntity).then(() => {
